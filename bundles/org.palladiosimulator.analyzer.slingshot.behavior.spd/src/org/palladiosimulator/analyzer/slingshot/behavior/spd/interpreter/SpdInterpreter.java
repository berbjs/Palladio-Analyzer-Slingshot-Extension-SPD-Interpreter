...
class SpdInterpreter extends SpdSwitch<SpdInterpreter.InterpretationResult> {
    ...
    public InterpretationResult caseSPD(final SPD spd) {
        ...
        spd.getTargetGroups()
            .stream()
            .forEach(target -> targetGroupStates.put(target, new TargetGroupState(target)));
    ...
    @Override
    public InterpretationResult caseReactiveScalingPolicy(ReactiveScalingPolicy policy) {
    ...
    public InterpretationResult caseModelBasedScalingPolicy(ModelBasedScalingPolicy policy) {
        // TODO IMPORTANT Auto-generated method stub
        LOGGER.debug("Interpreting ReactiveScalingPolicy Model " + policy.getEntityName() + "[" + policy.getId() + "]");
